...
function MagneticItem({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <motion.div className={className} onMouseMove={handleMouse} onMouseLeave={reset} animate={{ rotateX: pos.y, rotateY: -pos.x }} transition={{ type: "spring", stiffness: 200, damping: 20 }}>
      {children}
    </motion.div>
  );
}
...
export default function Header() {
  ...
  const links = [
    ...
    { name: t.nav.book, href: "/book-appointment" },
  ];
  ...
  return (
    <header className="bg-[#050d1a]/90 backdrop-blur-xl shadow-md sticky top-0 z-50">
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="lg:hidden bg-[#050d1a]/95 backdrop-blur-xl border-t border-blue-900/40">
            <div className="px-4 py-4 flex flex-col gap-2 text-white">
              ...
              <NavSignUp className="py-2 hover:text-[#38bdf8] transition-colors" onClick={() => setMobileOpen(false)} />
              <NavSignIn className="py-2 hover:text-[#38bdf8] transition-colors" onClick={() => setMobileOpen(false)} />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
